...
class SupplierMailer < ActionMailer::Base
  ...
  def welcome_email(supplier)
  	...
  	mail to: "<#{@supplier.email}>", bcc: "matriclick_notice@matriclick.com", subject: "[Matriclick.com] Bienvenido a la comunidad Matriclick"
  end
  ...
  def message_email_to_supplier(message, user)
    ...
    @country_url_path = @message.conversation.supplier_account.country.url_path
  	...
  	mail to: @email_to, bcc: "matriclick_notice@matriclick.com", subject: "[Matriclick.com] ¡Has recibido un mensaje!"
  end
end
